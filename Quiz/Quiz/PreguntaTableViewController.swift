...
class PreguntaTableViewController: UITableViewController, UISearchResultsUpdating, UISearchBarDelegate {
    ...
    /**
     ...
     Elimina una pregunta de la base de datos dado un índice y la tabla.
     Dependiendo de si hay una búsqueda activa habrá que extraer una pregunta de un sitio u otro.

     */
    func eliminaPregunta(tableView: UITableView, indexPath: IndexPath) {
        if estaFiltrando() {
